...
class LALossBinary(nn.Module):
    def __init__(self, cls_num_list, tau=1.0):
        ...
        base_probs = cls_num_list / cls_num_list.sum()  # 类别的先验概率
        scaled_class_weights = tau * torch.log(base_probs + 1e-12)  # 计算权重
        self.pos_weight = scaled_class_weights[1].float().cuda()  # 正类权重
        self.neg_weight = scaled_class_weights[0].float().cuda()  # 负类权重

    def forward(self, logits, target):
        # logits 形状：[batch_size, 1]
        # target 形状：[batch_size]
        logits = logits.squeeze(-1)  # 去掉最后一维
...
class TextCNN(nn.Module):
    ...
    def forward(self, x):  # [4, 3, 768]
        ...
        pooled = [F.max_pool1d(conv, conv.shape[2]).squeeze(2)
                  for conv in conved]
        # conv，形状为 [batch_size, n_filters, sent_len - filter_size + 1]。
...
class Model(nn.Module):
    ...
    def forward(self, seq_ids=None, input_ids=None, labels=None):
        ...
        #     # loss = loss.mean()  # 取平均损失（批次中所有样本的损失），并取负值
            return loss, prob
        else:
            return prob
