...
# from model_compare_1_transformer import Model
# from model_comparse_2_cnn_lstm import Model
...
def convert_examples_to_features(js, tokenizer, args):
    ...
    path_tokens1 = js['path']
    all_seq_ids = []  
    for seq in path_tokens1:  # for i in 3
    ...
    return InputFeatures(source_tokens, source_ids, all_seq_ids, js['id'], js['target'])
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path=None):
        ...
        if 'train' in file_path:
            for id, example in enumerate(self.examples[:1]):
                logger.info("*** Example ***")
                logger.info("idx: {}".format(id))
                logger.info("label: {}".format(example.label))
                logger.info("input_tokens: {}".format([x.replace('\u0120', '_') for x in example.input_tokens]))
                logger.info("input_ids: {}".format(' '.join(map(str, example.input_ids))))
                # logger.info("path_source: {}".format(example.path_source)) 
...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(bar):  # train_dataloader
            inputs = batch[0].to(args.device)  # 源码ids
            labels = batch[1].to(args.device)  # 标签
            seq_inputs = batch[2].to(args.device)  # 路径ids
            model.train()
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    checkpoint_prefix = 'checkpoint-best-acc/all_model'
                    output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))
                    if not os.path.exists(output_dir):
                        os.makedirs(output_dir)
                    model_to_save = model.module if hasattr(model, 'module') else model
                    output_dir = os.path.join(output_dir, '{}'.format(idx) + '{}'.format('_model.bin'))
                    torch.save(model_to_save.state_dict(), output_dir)
                    logger.info("Saving model checkpoint to %s", output_dir)
...
def evaluate(args, model, tokenizer, eval_when_training=False):
    ...
    if not os.path.exists(args.output_dir) and args.local_rank in [-1, 0]:  # local_rank是分布式训练的参数 多机多卡的分布式训练中分配id 0~n-1
        os.makedirs(args.output_dir)

    # Note that DistributedSampler samples randomly  local_rank=-1指没有启用分布式训练
...
def main():
    ...
    if args.do_train:
        ...
        train_dataset = TextDataset(tokenizer, args, args.train_data_file)  # input_ids、label、path_source源码id，类别，路径id
        if args.local_rank == 0:
    ...
    if args.do_test and args.local_rank in [-1, 0]:
        ...
        model.load_state_dict(torch.load(output_dir))
        model.to(args.device)
        result1 = test(args, model, tokenizer)
        logger.info("***** Test results *****")
        for key in sorted(result1.keys()):
            logger.info("  %s = %s", key, str(round(result1[key], 4)))
...
# python run.py --output_dir=./saved_models --model_type=roberta --tokenizer_name=../models/codebert --model_name_or_path=../models/codebert --do_train --train_data_file=../dataset/cdata/nobalance/train.jsonl --eval_data_file=../dataset/cdata/nobalance/valid.jsonl --test_data_file=../dataset/cdata/nobalance/test.jsonl --epoch 8 --block_size 400 --train_batch_size 40 --eval_batch_size 64 --learning_rate 2e-5 --max_grad_norm 1.0 --evaluate_during_training --seed 123456 --cnn_size 128 --filter_size 3 --d_size 128 --pkl_file=short_3path_cdata_nobalance.pkl